...
// TKMCacheClusterReconciler reconciles a TKMCacheCluster object
type TKMCacheClusterReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// +kubebuilder:rbac:groups=tkm.io,resources=tkmcacheclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tkm.io,resources=tkmcacheclusters/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=tkm.io,resources=tkmcacheclusters/finalizers,verbs=update

// Reconcile is part of the main Kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *TKMCacheClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)

	var clusterCache tkmv1alpha1.TKMCacheCluster
	if err := r.Get(ctx, req.NamespacedName, &clusterCache); err != nil {
		logger.Error(err, "unable to fetch TKMCacheCluster")
	...
	img, err := fetcher.FetchImg(clusterCache.Spec.Image)
	if err != nil {
...
func (r *TKMCacheClusterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&tkmv1alpha1.TKMCacheCluster{}).
		Complete(r)
}

// Helper function to set conditions on the cluster cache
func setClusterCondition(obj *tkmv1alpha1.TKMCacheCluster, condType string, status metav1.ConditionStatus, reason, msg string) {
